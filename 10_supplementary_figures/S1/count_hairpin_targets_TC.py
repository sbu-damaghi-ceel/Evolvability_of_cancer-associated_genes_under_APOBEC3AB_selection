...
# find C of TpC and G of GpA indices
def find_C_indices_TpC(seq):
    patterns = ["TC"]
...
def find_G_indices_GpA(seq):
    patterns = ["GA"]
    patterns_regex = re.compile('|'.join('(?={0})'.format(re.escape(p)) for p in patterns))

    return [match.start() for match in patterns_regex.finditer(seq)]

# count APOBEC target TpC in hairpins
...
def search_hairpin_apobec_target(seq, looplen = 3, target_pos_in_loop = 3, maxstem = 20, threshold = 15): 
    ...
    indices_plus  = find_C_indices_TpC(seq)
    indices_minus = find_C_indices_TpC(rev_seq)
...
def get_CDUR_stats(seq, looplen = 3, target_pos_in_loop = 3, maxstem = 20, threshold = 15):
    ...
    # a. number of target
    T_C_loop = len(indices)
    # b. number of nonsynonymous
    nonsynonymity = list(map(is_C_to_T_nonsynonymous, T_C_loop*[seq], indices))
    repTrT_C_loop = nonsynonymity.count(False)
    # c. b/a
    if T_C_loop == 0:
        repTrFracT_C_loop = 0
    else:
        repTrFracT_C_loop = repTrT_C_loop/T_C_loop

    return (T_C_loop, repTrT_C_loop, repTrFracT_C_loop)
...
def get_CDUR_stats_from_fasta(file_path, *args, **kwargs):
    ...
    for seq_record in SeqIO.parse(file_path, "fasta"):
        T_C_loop, repTrT_C_loop, repTrFracT_C_loop = get_CDUR_stats(str(seq_record.seq), looplen, target_pos_in_loop, maxstem, threshold)

        T_C_loops.append(T_C_loop)
        repTrT_C_loops.append(repTrT_C_loop) 
        repTrFracT_C_loops.append(repTrFracT_C_loop)

    belowT_C_loop = np.sum(np.array(T_C_loops[1:]) < T_C_loops[0])/1000
    repTr_belowT_C_loop = np.sum(np.array(repTrT_C_loops[1:]) < repTrT_C_loops[0])/1000
    repTrFrac_belowT_C_loop = np.sum(np.array(repTrFracT_C_loops[1:]) < repTrFracT_C_loops[0])/1000

    if args:
        return args, belowT_C_loop, repTr_belowT_C_loop, repTrFrac_belowT_C_loop
    else:
        return belowT_C_loop, repTr_belowT_C_loop, repTrFrac_belowT_C_loop
...
outfile_path = PurePath(FILE_PATH, "cdur.gencode.v40.pc_transcripts.nopary.cdsonly.refseq.tcloop.csv")
