...
ncbi_ptroglodytes_file = Path(PurePath(base_path, "ncbi_ptroglodytes_clean.tsv"))

ncbi_ptroglodytes = pd.read_csv(ncbi_ptroglodytes_file, sep="\t")
gene_ids = ncbi_ptroglodytes.loc[ncbi_ptroglodytes["type"] == "PROTEIN_CODING"]["gene_id"].to_list()
        ...
        for idx in range(len(subrecord)):
            ...
            if cds_indices:
                ...
                if transcript_variant:
                    ...
                    new_id = f"{ref_id}_Pan_troglodytes_{gene_symbol}_{'_'.join(transcript_variant.split())}"
                else:
                    if "/" in gene_symbol:
                        gene_symbol = gene_symbol.replace("/", ".")

                    new_id = f"{ref_id}_Pan_troglodytes_{gene_symbol}"
                ...
                if "N" in subrecord[idx]["GBSeq_sequence"][from_idx-1:to_idx].upper():
                    ...
                    SeqIO.write(seq_record, PurePath(base_path, "pan_troglodytes_genes", f"{new_id}.fa"), "fasta")
...
output_file = PurePath(base_path, f"pan_troglodytes_pc_transcripts.fa")
