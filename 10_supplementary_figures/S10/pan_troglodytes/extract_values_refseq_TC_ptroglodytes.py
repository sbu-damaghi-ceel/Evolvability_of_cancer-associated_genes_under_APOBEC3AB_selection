...
# fasta file input
refseq_file = Path(PurePath(base_path, f"pan_troglodytes_pc_transcripts.fa"))
...
for cnt, seq_record in enumerate(SeqIO.parse(refseq_file, "fasta")):    
    ...
    run1_file = Path(PurePath(base_path, f"pan_troglodytes", f"cdur_run1", filename))
    run2_file = Path(PurePath(base_path, f"pan_troglodytes", f"cdur_run2", filename))
    run3_file = Path(PurePath(base_path, f"pan_troglodytes", f"cdur_run3", filename))
...
df.to_csv(Path(PurePath(base_path, f"cdur.pan_troglodytes_pc_transcripts.tc.csv")), index=False)
