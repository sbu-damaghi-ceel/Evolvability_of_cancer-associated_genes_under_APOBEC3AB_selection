...
input_file_name = f"gencode.vM{release_num}.pc_transcripts.nopary.cdsonly.fa"
input_file = Path(PurePath(base_path, input_file_name))

canonical_file_name = f"gencode.vM{release_num}.pc_transcripts.nopary.cdsonly.canonical.id.csv"
canonical_file = Path(PurePath(base_path, canonical_file_name))
canonical = pd.read_csv(canonical_file)

refseq_file_name = f"gencode.vM{release_num}.pc_transcripts.nopary.cdsonly.haverefseq.id.csv"
refseq_file = Path(PurePath(base_path, refseq_file_name))
refseq = pd.read_csv(refseq_file)

output_file_canonical_name = f"gencode.vM{release_num}.pc_transcripts.nopary.cdsonly.canonical.fa"
output_file_canonical = Path(PurePath(base_path, output_file_canonical_name))

output_file_refseq_name = f"gencode.vM{release_num}.pc_transcripts.nopary.cdsonly.refseq.fa"
