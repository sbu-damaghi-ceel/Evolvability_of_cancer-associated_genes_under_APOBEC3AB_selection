...
cdur_file <- paste(base_path, "cdur.gencode.vM33.pc_transcripts.nopary.cdsonly.refseq.gc.csv", sep = "")
...
out_file <- paste(base_path, "cdur_plot_mouse_refseq_transcripts_gc.tiff", sep="")
ggsave(out_file, plot = cdur_plot, dpi = 600, width = 134, height = 134, device='tiff', units = "mm")

out_file <- paste(base_path, "cdur_plot_mouse_refseq_transcripts_gc.png", sep="")
ggsave(out_file, plot = cdur_plot, dpi = 1200, width = 85, height = 85, units = "mm")

out_file <- paste(base_path, "cdur_plot_mouse_refseq_transcripts_small_gc.png", sep="")
