    ...
    if path.is_dir():
        ...
        if genename in control_genes_having_ten_orthologs["Gene_name"].to_list(): 
            ...
            for i, species in enumerate(species_of_interest):
                ...
                seq_record = SeqRecord(Seq(record[0]["GBSeq_sequence"][from_idx-1:to_idx].upper()), id=new_id, description="")
                seq_records.append(seq_record)
            
            output_file = PurePath(str(path), f"ten_species_{genename.lower()}_cdur.fa")
