library(biomaRt)
library(dplyr)

# use GRCh38 to get information
mart38 <- useEnsembl(biomart="ensembl", dataset = "hsapiens_gene_ensembl")

# set attributes to view
# you can see possible attributes using: listAttributes(mart38)
attributes.of.interest = c(
  "external_gene_name",
  "ensembl_transcript_id_version",
  "ensembl_transcript_id",
  "ensembl_gene_id",
  "description",
  "gene_biotype",
  "transcript_gencode_basic",
  "transcript_appris",
  "transcript_is_canonical",
  "transcript_mane_select",
  "refseq_mrna"
)

# import name and id of genes from gencode (cleaned for valid sequence)
base_path <- "C:/Users/CEEL-PC-005/Desktop/Joon/Final_scripts/Cancer_genes_analysis_of_APOBEC_motifs_test/"
name_id_file <- paste(base_path, "gencode.v40.pc_transcripts.nopary.cdsonly.name.id.csv", sep="")
genename_id <- read.csv(name_id_file, sep = ",")

# retrieve information from biomart
rlt <- getBM(
  attributes = attributes.of.interest,
  filters = "ensembl_transcript_id",
  values = genename_id$Ensembl_transcript_id,
  mart = mart38
)

# get only distinct gene id
rlt_distinct <- rlt %>% distinct(ensembl_transcript_id, .keep_all = TRUE)

# get transcripts marked as canonical
rlt_canonical <- rlt_distinct %>% filter(transcript_is_canonical == 1)
write.csv2(rlt_canonical$ensembl_transcript_id, paste(base_path, "gencode.v40.pc_transcripts.nopary.cdsonly.canonical.id.csv", sep=""), row.names = FALSE, quote=FALSE)

# get transcript having refseq id
rlt_refseq <- rlt_distinct %>% filter(refseq_mrna != "")

# get ids not found from GRCh38
id_not_found <- setdiff(genename_id$Ensembl_transcript_id, rlt_distinct$ensembl_transcript_id)

# use GRCh38 to get information
mart37 <- useEnsembl(biomart="ensembl", dataset = "hsapiens_gene_ensembl", GRCh=37)

# set attributes to view
# several attributes removed because they are not available in GRCh37
attributes.of.interest37 = c(
  "external_gene_name",
  "ensembl_transcript_id_version",
  "ensembl_transcript_id",
  "ensembl_gene_id",
  "description",
  "gene_biotype",
  "transcript_gencode_basic",
  "refseq_mrna"
)

# retrieve information from biomart
# only sumbint ids not found previously
rlt37 <- getBM(
  attributes = attributes.of.interest37,
  filters = "ensembl_transcript_id",
  values = id_not_found,
  mart = mart37
)

# get only distinct gene id
rlt37_distinct <- rlt37 %>% distinct(ensembl_transcript_id, .keep_all = TRUE)

# get transcripts having refseq id
rlt_refseq37 <- rlt37_distinct %>% filter(refseq_mrna != "")

# combine transcripts having refseq id
rlt_refseq_all <- c(rlt_refseq$ensembl_transcript_id, rlt_refseq37$ensembl_transcript_id)
write.csv(rlt_refseq_all, paste(base_path, "gencode.v40.pc_transcripts.nopary.cdsonly.haverefseq.id.csv", sep=""), row.names = FALSE, quote=FALSE)

# check how many transcripts id were ignored
id_yet_not_found <- setdiff(id_not_found, rlt37_distinct$ensembl_transcript_id)
length(id_yet_not_found)
